...
def test_format_resource_id() -> None:
    ...
    result = format_resource_id("proj-test", "postgres", "database", "my-db")
    assert result == "proj-test/postgres/database/my-db"


def test_format_resource_id_rejects_project_with_slash() -> None:
    """format_resource_id rejects project_id containing a slash."""
    import pytest

    with pytest.raises(ValueError, match="must not contain '/'"):
        format_resource_id("bad/proj", "postgres", "database", "my-db")


def test_format_resource_id_rejects_empty_project() -> None:
    """format_resource_id rejects empty project_id."""
    import pytest
...
def test_format_internal_resource_id() -> None:
    ...
    result = format_internal_resource_id("proj-test", "postgres", "database", "analytics")
    assert result == "resource:proj-test_postgres_database_analytics"
...
def test_format_internal_resource_id_slash_normalization() -> None:
    ...
    result = format_internal_resource_id("proj-test", "gcp", "cloudsql/instance", "my-db")
    assert result == "resource:proj-test_gcp_cloudsql_instance_my-db"


def test_resource_reference_id_property() -> None:
    """ResourceReference.id returns formatted resource ID."""
    ref = ResourceReference(project_id="proj-test", provider="postgres", resource="database", name="my-db")
    assert ref.id == "proj-test/postgres/database/my-db"
...
def test_owner_reference_initialization() -> None:
    ...
    ref = OwnerReference(project_id="proj-test", provider="app", resource="server", name="my-server")
...
def test_owner_reference_id_property() -> None:
    ...
    ref = OwnerReference(project_id="proj-test", provider="app", resource="server", name="my-server")
    assert ref.id == "proj-test/app/server/my-server"
...
def test_owner_reference_equality() -> None:
    ...
    ref1 = OwnerReference(project_id="proj-test", provider="app", resource="server", name="my-server")
    ref2 = OwnerReference(project_id="proj-test", provider="app", resource="server", name="my-server")
    ref3 = OwnerReference(project_id="proj-test", provider="app", resource="server", name="other-server")
...
def test_owner_reference_not_equal_to_resource_reference() -> None:
    ...
    owner_ref = OwnerReference(project_id="proj-test", provider="app", resource="server", name="my-server")
    resource_ref = ResourceReference(project_id="proj-test", provider="app", resource="server", name="my-server")
...
def test_resource_id_property(stub_resource: StubResource) -> None:
    ...
    assert stub_resource.id == "proj-test/test/stub/my-resource"
...
def test_dependency_id_property() -> None:
    ...
    assert dep.id == "proj-test/postgres/database/my-db"
...
def test_dependency_in_config() -> None:
    ...
    assert config.database.id == "proj-test/test/stub/my-db"
...
async def test_apply_auto_sets_owner_from_context(stub_resource: StubResource) -> None:
    ...
    parent_owner = OwnerReference(project_id="proj-test", provider="test", resource="parent", name="my-parent")
